...
class Input
{
    ...
    public function set_fields($id, $fields)
    {
        ...
        if (isset($fields->name)) {
            if (preg_replace('/[^\p{N}\p{L}_\s\-]/u', '', $fields->name)!=$fields->name) {
                return array('success'=>false, 'message'=>'invalid characters in input name');
            }
        ...
        if (isset($fields->description)) {
            if (preg_replace('/[^\p{N}\p{L}_\s\-.]/u', '', $fields->description)!=$fields->description) {
                return array('success'=>false, 'message'=>'invalid characters in input description');
            }
    ...
    private function redis_get_inputs($userid)
    {
        ...
        foreach ($result as $row) {
            ...
            if (!isset($dbinputs[$row['nodeid']])) {
                $dbinputs[$row['nodeid']] = array();
            }
    ...
    {
        $userid = (int) $userid;
        if (!$this->redis->exists("user:inputs:$userid")) {
            $this->load_to_redis($userid);
        }
    ...
    public function get_name($id)
    {
        ...
        if ($this->redis) {
            if (!$this->redis->exists("input:$id")) {
                $this->load_input_to_redis($id);
            }
    ...
    public function delete_multiple($userid, $inputids)
    {
        foreach ($inputids as $inputid) {
            if ($this->belongs_to_user($userid, $inputid)) {
        ...
        while ($row = $result->fetch_object()) {
            ...
            for ($i=0; $i<count($pairs); $i++) {
                ...
                if (count($keyarg)==2) {
                    ...
                    if (isset($process_class->process_map[$key])) {
                        $key = $process_class->process_map[$key];
                    }
