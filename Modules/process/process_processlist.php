...
class Process_ProcessList
{
    ...
    public function core_process_list()
    {
        ...
        $list[1] = array(dgettext("process_messages","Log to feed "),ProcessArg::FEEDID,"log_to_feed",1,DataType::REALTIME,dgettext("process_messages","Main"),array(Engine::PHPFINA,Engine::PHPFIWA,Engine::PHPTIMESERIES,Engine::MYSQL,Engine::MYSQLMEMORY,Engine::CASSANDRA), 'nochange'=>true, 'desc'=>"".dgettext("process_messages",'<p><b>Log to feed:</b> This processor logs to a timeseries feed which can then be used to explore historic data. This is recommended for logging power, temperature, humidity, voltage and current data.</p><p><b>Feed engine:</b><ul><li><b>PHPFina</b> is the recommended feed engine it is a basic fixed interval timeseries engine.</li><li><b>PHPTimeseries</b> is for data posted at a non regular interval such as on state change.</li></ul></p><p><b>Feed interval:</b> When selecting the feed interval select an interval that is the same as, or longer than the update rate that is set in your monitoring equipment. Setting the interval rate to be shorter than the update rate of the equipment causes un-needed disk space to be used up.</p>'),'short_name'=>'log');
        $list[2] = array(dgettext("process_messages","x"),ProcessArg::VALUE,"scale",0,DataType::UNDEFINED,dgettext("process_messages","Calibration"), 'desc'=>"".dgettext("process_messages",'<p>Multiplies current value by given constant. This can be useful for calibrating a particular variable on the web rather than by reprogramming hardware.</p>'),'short_name'=>'x');
        $list[3] = array(dgettext("process_messages","+"),ProcessArg::VALUE,"offset",0,DataType::UNDEFINED,dgettext("process_messages","Calibration"), 'desc'=>"".dgettext("process_messages",'<p>Offset current value by given value. This can again be useful for calibrating a particular variable on the web rather than by reprogramming hardware.</p>'),'short_name'=>'+');
        $list[4] = array(dgettext("process_messages","Power to kWh"),ProcessArg::FEEDID,"power_to_kwh",1,DataType::REALTIME,dgettext("process_messages","Main"),array(Engine::PHPFINA,Engine::PHPTIMESERIES,Engine::MYSQL,Engine::MYSQLMEMORY), 'nochange'=>true, 'desc'=>"".dgettext("process_messages",'<p><b>Power to kWh:</b> Convert a power value in Watts to a cumulative kWh feed.<br><br><b>Visualisation tip:</b> Feeds created with this input processor can be used to generate daily kWh data using the BarGraph visualisation with the delta property set to 1.<br>See forum thread here for an example <a href="https://openenergymonitor.org/emon/node/12308">Creating kWh per day bar graphs from Accumulating kWh </a></p>'),'short_name'=>'kwh');
        $list[5] = array(dgettext("process_messages","Power to kWh/d"),ProcessArg::FEEDID,"power_to_kwhd",1,DataType::DAILY,dgettext("process_messages","Power & Energy"),array(Engine::PHPTIMESERIES,Engine::MYSQL,Engine::MYSQLMEMORY), 'nochange'=>true, 'desc'=>"".dgettext("process_messages",'<p>Convert a power value in Watts to a feed that contains an entry for the total energy used each day (kWh/d)</p>'),'short_name'=>'kwhd');
        $list[6] = array(dgettext("process_messages","x input"),ProcessArg::INPUTID,"times_input",0,DataType::UNDEFINED,dgettext("process_messages","Input"), 'desc'=>"".dgettext("process_messages",'<p>Multiplies the current value with the last value from other input as selected from the input list.</p>'),'short_name'=>'x inp');
        $list[7] = array(dgettext("process_messages","Input on-time"),ProcessArg::FEEDID,"input_ontime",1,DataType::DAILY,dgettext("process_messages","Input"),array(Engine::PHPTIMESERIES,Engine::MYSQL,Engine::MYSQLMEMORY), 'nochange'=>true, 'desc'=>"".dgettext("process_messages",'<p>Counts the amount of time that an input is high in each day and logs the result to a feed. Created for counting the number of hours a solar hot water pump is on each day</p>'),'short_name'=>'ontime');
        $list[8] = array(dgettext("process_messages","Wh increments to kWh/d"),ProcessArg::FEEDID,"whinc_to_kwhd",1,DataType::DAILY,dgettext("process_messages","Power & Energy"),array(Engine::PHPTIMESERIES,Engine::MYSQL,Engine::MYSQLMEMORY), 'nochange'=>true, 'desc'=>"".dgettext("process_messages",'<p>Accumulate Wh measurements into kWh/d.<p><b>Input</b>: energy increments in Wh.</p>'),'short_name'=>'kwhinckwhd');
        $list[9] = array(dgettext("process_messages","kWh to kWh/d (OLD)"),ProcessArg::FEEDID,"kwh_to_kwhd_old",1,DataType::DAILY,"Deleted",array(Engine::PHPTIMESERIES), 'desc'=>"",'short_name'=>'kwhkwhd');
        $list[10] = array(dgettext("process_messages","Upsert feed at day"),ProcessArg::FEEDID,"update_feed_data",1,DataType::DAILY,dgettext("process_messages","Input"),array(Engine::MYSQL,Engine::MYSQLMEMORY), 'nochange'=>true, 'desc'=>"".dgettext("process_messages",'<p>Updates or inserts daily value on the specified time (given by the JSON time parameter from the API) of the specified feed</p>'),'short_name'=>'update');
        $list[11] = array(dgettext("process_messages","+ input"),ProcessArg::INPUTID,"add_input",0,DataType::UNDEFINED,dgettext("process_messages","Input"), 'desc'=>"".dgettext("process_messages",'<p>Adds the current value with the last value from other input as selected from the input list. The result is passed back for further processing by the next processor in the processing list.</p>'),'short_name'=>'+ inp');
        $list[12] = array(dgettext("process_messages","/ input"),ProcessArg::INPUTID,"divide_input",0,DataType::UNDEFINED,dgettext("process_messages","Input"), 'desc'=>"".dgettext("process_messages",'<p>Divides the current value with the last value from other input as selected from the input list. The result is passed back for further processing by the next processor in the processing list.</p>'),'short_name'=>'/ inp');
        $list[13] = array(dgettext("process_messages","Phaseshift"),ProcessArg::VALUE,"phaseshift",0,DataType::UNDEFINED,"Deleted", 'desc'=>"",'short_name'=>'phaseshift');
        $list[14] = array(dgettext("process_messages","Accumulator"),ProcessArg::FEEDID,"accumulator",1,DataType::REALTIME,dgettext("process_messages","Misc"),array(Engine::PHPFINA,Engine::PHPTIMESERIES,Engine::MYSQL,Engine::MYSQLMEMORY), 'desc'=>"".dgettext("process_messages",'<p>Output feed accumulates by input value</p>'),'short_name'=>'accumulate');
        $list[15] = array(dgettext("process_messages","Rate of change"),ProcessArg::FEEDID,"ratechange",1,DataType::REALTIME,dgettext("process_messages","Misc"),array(Engine::PHPFIWA,Engine::PHPFINA,Engine::PHPTIMESERIES), 'requireredis'=>true, 'desc'=>"".dgettext("process_messages",'<p>Output feed is the difference between the current value and the last</p>'),'short_name'=>'rate');
        $list[16] = array(dgettext("process_messages","Histogram"),ProcessArg::FEEDID,"histogram",2,DataType::HISTOGRAM,dgettext("process_messages","Power & Energy"),array(Engine::MYSQL,Engine::MYSQLMEMORY), 'desc'=>"".dgettext("process_messages",'Creates a histogram of energy binned by power ranges. For each power range on the x-axis, this processor will aggregate the total energy of the stream while it was in that power range.<p><b>Input</b>: power in Watts.</p>'),'short_name'=>'hist');
        $list[17] = array(dgettext("process_messages","Daily Average"),ProcessArg::FEEDID,"average",2,DataType::HISTOGRAM,"Deleted",array(Engine::PHPTIMESERIES), 'desc'=>"",'short_name'=>'average');

        // to be reintroduced in post-processing
        $list[18] = array(dgettext("process_messages","Heat flux"),ProcessArg::FEEDID,"heat_flux",1,DataType::REALTIME,"Deleted",array(Engine::PHPFIWA,Engine::PHPFINA,Engine::PHPTIMESERIES), 'desc'=>"",'short_name'=>'flux');

        // need to remove - result can be achieved with allow_positive & power_to_kwhd
        $list[19] = array(dgettext("process_messages","Power gained to kWh/d"),ProcessArg::FEEDID,"power_acc_to_kwhd",1,DataType::DAILY,"Deleted",array(Engine::PHPTIMESERIES), 'desc'=>"",'short_name'=>'pwrgain');

        // - look into implementation that doesnt need to store the ref feed
        $list[20] = array(dgettext("process_messages","Total pulse count to pulse increment"),ProcessArg::FEEDID,"pulse_diff",1,DataType::REALTIME,dgettext("process_messages","Pulse"),array(Engine::PHPFINA,Engine::PHPTIMESERIES), 'desc'=>"".dgettext("process_messages",'<p>Returns the number of pulses incremented since the last update for a input that is a cumulative pulse count. i.e If the input updates from 23400 to 23410 the result will be an incremenet of 10.</p>'),'short_name'=>'pulsdiff');
        // fixed works now with redis - look into state implementation without feed
        $list[21] = array(dgettext("process_messages","kWh to Power"),ProcessArg::FEEDID,"kwh_to_power",1,DataType::REALTIME,dgettext("process_messages","Power & Energy"),array(Engine::PHPFIWA,Engine::PHPFINA,Engine::PHPTIMESERIES), 'requireredis'=>true, 'desc'=>"".dgettext("process_messages",'<p>Convert accumulating kWh to instantaneous power</p>'),'short_name'=>'kwhpwr');

        $list[22] = array(dgettext("process_messages","- input"),ProcessArg::INPUTID,"subtract_input",0,DataType::UNDEFINED,dgettext("process_messages","Input"), 'desc'=>"".dgettext("process_messages",'<p>Subtracts from the current value the last value from other input as selected from the input list.</p>'),'short_name'=>'- inp');
        $list[23] = array(dgettext("process_messages","kWh to kWh/d"),ProcessArg::FEEDID,"kwh_to_kwhd",2,DataType::DAILY,dgettext("process_messages","Power & Energy"),array(Engine::PHPTIMESERIES), 'requireredis'=>true, 'nochange'=>true, 'desc'=>"".dgettext("process_messages",'<p>Upsert kWh to a daily value.</p>'),'short_name'=>'kwhkwhd');
        $list[24] = array(dgettext("process_messages","Allow positive"),ProcessArg::NONE,"allowpositive",0,DataType::UNDEFINED,dgettext("process_messages","Limits"), 'desc'=>"".dgettext("process_messages",'<p>Negative values are zeroed for further processing by the next processor in the processing list.</p>'),'short_name'=>'> 0');
        $list[25] = array(dgettext("process_messages","Allow negative"),ProcessArg::NONE,"allownegative",0,DataType::UNDEFINED,dgettext("process_messages","Limits"), 'desc'=>"".dgettext("process_messages",'<p>Positive values are zeroed for further processing by the next processor in the processing list.</p>'),'short_name'=>'< 0');
        $list[26] = array(dgettext("process_messages","Signed to unsigned"),ProcessArg::NONE,"signed2unsigned",0,DataType::UNDEFINED,dgettext("process_messages","Misc"), 'desc'=>"".dgettext("process_messages",'<p>Convert a number that was interpreted as a 16 bit signed number to an unsigned number.</p>'),'short_name'=>'unsign');
        $list[27] = array(dgettext("process_messages","Max daily value"),ProcessArg::FEEDID,"max_value",1,DataType::DAILY,dgettext("process_messages","Misc"),array(Engine::PHPTIMESERIES,Engine::MYSQL,Engine::MYSQLMEMORY), 'nochange'=>true, 'desc'=>"".dgettext("process_messages",'<p>Maximal daily value. Upserts on the selected daily feed the highest value reached each day.</p>'),'short_name'=>'max');
        $list[28] = array(dgettext("process_messages","Min daily value"),ProcessArg::FEEDID,"min_value",1,DataType::DAILY,dgettext("process_messages","Misc"),array(Engine::PHPTIMESERIES,Engine::MYSQL,Engine::MYSQLMEMORY), 'nochange'=>true, 'desc'=>"".dgettext("process_messages",'<p>Minimal daily value. Upserts on the selected daily feed the lowest value reached each day.</p>'),'short_name'=>'min');

        $list[29] = array(dgettext("process_messages"," + feed"),ProcessArg::FEEDID,"add_feed",0,DataType::UNDEFINED,dgettext("process_messages","Feed"), 'desc'=>"".dgettext("process_messages",'<p>Adds the current value with the last value from a feed as selected from the feed list.</p>'),'short_name'=>'+ feed');
        $list[30] = array(dgettext("process_messages"," - feed"),ProcessArg::FEEDID,"sub_feed",0,DataType::UNDEFINED,dgettext("process_messages","Feed"), 'desc'=>"".dgettext("process_messages",'<p>Subtracts from the current value the last value from a feed as selected from the feed list.</p>'),'short_name'=>'- feed');
        $list[31] = array(dgettext("process_messages"," * feed"),ProcessArg::FEEDID,"multiply_by_feed",0,DataType::UNDEFINED,dgettext("process_messages","Feed"), 'desc'=>"".dgettext("process_messages",'<p>Multiplies the current value with the last value from a feed as selected from the feed list.</p>'),'short_name'=>'x feed');
        $list[32] = array(dgettext("process_messages"," / feed"),ProcessArg::FEEDID,"divide_by_feed",0,DataType::UNDEFINED,dgettext("process_messages","Feed"), 'desc'=>"".dgettext("process_messages",'<p>Divides the current value by the last value from a feed as selected from the feed list.</p>'),'short_name'=>'/ feed');
        $list[33] = array(dgettext("process_messages","Reset to ZERO"),ProcessArg::NONE,"reset2zero",0,DataType::UNDEFINED,dgettext("process_messages","Misc"), 'desc'=>"".dgettext("process_messages",'<p>The value "0" is passed back for further processing by the next processor in the processing list.</p>'),'short_name'=>'= 0');

        $list[34] = array(dgettext("process_messages","Wh Accumulator"),ProcessArg::FEEDID,"wh_accumulator",1,DataType::REALTIME,dgettext("process_messages","Main"),array(Engine::PHPFINA,Engine::PHPTIMESERIES), 'requireredis'=>true, 'desc'=>"".dgettext("process_messages",'<b>Wh Accumulator:</b> Use with emontx, emonth or emonpi pulsecount or an emontx running firmware <i>emonTxV3_4_continuous_kwhtotals</i> sending cumulative watt hours.<br><br>This processor ensures that when the emontx is reset the watt hour count in emoncms does not reset, it also checks filter\'s out spikes in energy use that are larger than a max power threshold set in the processor, assuming these are error\'s, the max power threshold is set to 25kW. <br><br><b>Visualisation tip:</b> Feeds created with this input processor can be used to generate daily kWh data using the BarGraph visualisation with the delta property set to 1 and scale set to 0.001.<br>See forum thread here for an example <a href="https://openenergymonitor.org/emon/node/12308" Creating kWh per day bar graphs from Accumulating kWh </a></p>'),'short_name'=>'whacc');

        $list[35] = array(dgettext("process_messages","Publish to MQTT"),ProcessArg::TEXT,"publish_to_mqtt",1,DataType::UNDEFINED,dgettext("process_messages","Misc"), 'nochange'=>true, 'desc'=>"".dgettext("process_messages",'<p>Publishes value to MQTT topic e.g. \'home/power/kitchen\'</p>'),'short_name'=>'MQTT');

        $list[36] = array(dgettext("process_messages","Reset to NULL"),ProcessArg::NONE,"reset2null",0,DataType::UNDEFINED,dgettext("process_messages","Misc"), 'desc'=>"".dgettext("process_messages",'<p>Value is set to NULL.</p><p>Useful for conditional process to work on.</p>'),'short_name'=>'null');
        $list[37] = array(dgettext("process_messages","Reset to Original"),ProcessArg::NONE,"reset2original",0,DataType::UNDEFINED,dgettext("process_messages","Misc"), 'desc'=>"".dgettext("process_messages",'<p>The value is set to the original value at the start of the process list.</p>'),'short_name'=>'ori');

        $list[42] = array(dgettext("process_messages","If ZERO, skip next"),ProcessArg::NONE,"if_zero_skip",0,DataType::UNDEFINED,dgettext("process_messages","Conditional"), 'nochange'=>true, 'desc'=>"".dgettext("process_messages",'<p>If value from last process is ZERO, process execution will skip execution of next process in list.</p>'),'short_name'=>'0? skip');
        $list[43] = array(dgettext("process_messages","If !ZERO, skip next"),ProcessArg::NONE,"if_not_zero_skip",0,DataType::UNDEFINED,dgettext("process_messages","Conditional"), 'nochange'=>true, 'desc'=>"".dgettext("process_messages",'<p>If value from last process is NOT ZERO, process execution will skip execution of next process in list.</p>'),'short_name'=>'!0? skip');
        $list[44] = array(dgettext("process_messages","If NULL, skip next"),ProcessArg::NONE,"if_null_skip",0,DataType::UNDEFINED,dgettext("process_messages","Conditional"), 'nochange'=>true, 'desc'=>"".dgettext("process_messages",'<p>If value from last process is NULL, process execution will skip execution of next process in list.</p>'),'short_name'=>'N? skip');
        $list[45] = array(dgettext("process_messages","If !NULL, skip next"),ProcessArg::NONE,"if_not_null_skip",0,DataType::UNDEFINED,dgettext("process_messages","Conditional"), 'nochange'=>true, 'desc'=>"".dgettext("process_messages",'<p>If value from last process is NOT NULL, process execution will skip execution of next process in list.</p>'),'short_name'=>'!N? skip');

        $list[46] = array(dgettext("process_messages","If >, skip next"),ProcessArg::VALUE,"if_gt_skip",0,DataType::UNDEFINED,dgettext("process_messages","Conditional - User value"), 'nochange'=>true, 'desc'=>"".dgettext("process_messages",'<p>If value from last process is greater than the specified value, process execution will skip execution of next process in list.</p>'),'short_name'=>'>? skip');
        $list[47] = array(dgettext("process_messages","If >=, skip next"),ProcessArg::VALUE,"if_gt_equal_skip",0,DataType::UNDEFINED,dgettext("process_messages","Conditional - User value"), 'nochange'=>true, 'desc'=>"".dgettext("process_messages",'<p>If value from last process is greater or equal to the specified value, process execution will skip execution of next process in list.</p>'),'short_name'=>'>=? skip');
        $list[48] = array(dgettext("process_messages","If <, skip next"),ProcessArg::VALUE,"if_lt_skip",0,DataType::UNDEFINED,dgettext("process_messages","Conditional - User value"), 'nochange'=>true, 'desc'=>"".dgettext("process_messages",'<p>If value from last process is lower than the specified value, process execution will skip execution of next process in list.</p>'),'short_name'=>'<? skip');
        $list[49] = array(dgettext("process_messages","If <=, skip next"),ProcessArg::VALUE,"if_lt_equal_skip",0,DataType::UNDEFINED,dgettext("process_messages","Conditional - User value"), 'nochange'=>true, 'desc'=>"".dgettext("process_messages",'<p>If value from last process is lower or equal to the specified value, process execution will skip execution of next process in list.</p>'),'short_name'=>'<=? skip');
        $list[50] = array(dgettext("process_messages","If =, skip next"),ProcessArg::VALUE,"if_equal_skip",0,DataType::UNDEFINED,dgettext("process_messages","Conditional - User value"), 'nochange'=>true, 'desc'=>"".dgettext("process_messages",'<p>If value from last process is equal to the specified value, process execution will skip execution of next process in list.</p>'),'short_name'=>'=? skip');
        $list[51] = array(dgettext("process_messages","If !=, skip next"),ProcessArg::VALUE,"if_not_equal_skip",0,DataType::UNDEFINED,dgettext("process_messages","Conditional - User value"), 'nochange'=>true, 'desc'=>"".dgettext("process_messages",'<p>If value from last process is NOT equal to the specified value, process execution will skip execution of next process in list.</p>'),'short_name'=>'!=? skip');

        // A bit or warning: if user goto's in loop, the php will lock until the server defined timesout with an error
        $list[52] = array(dgettext("process_messages","GOTO"),ProcessArg::VALUE,"goto_process",0,DataType::UNDEFINED,dgettext("process_messages","Misc"), 'nochange'=>true, 'desc'=>"".dgettext("process_messages",'<p>Jumps the process execution to the specified position.</p><p><b>Warning</b><br>If you\'re not careful you can create a goto loop on the process list.<br>When a loop occurs, the API will appear to lock until the server php times out with an error.</p>'),'short_name'=>'GOTO');
        
        // $list[29] = array(dgettext("process_messages","save to input"),ProcessArg::INPUTID,"save_to_input",1,DataType::UNDEFINED);
        ...
        $list[53] = array(dgettext("process_messages","Source Feed"),ProcessArg::FEEDID,"source_feed_data_time",1,DataType::UNDEFINED,dgettext("process_messages","Virtual"), 'desc'=>"".dgettext("process_messages",'<p><b>Source Feed:</b><br>Virtual feeds should use this processor as the first one in the process list. It sources data from the selected feed.<br>The sourced value is passed back for further processing by the next processor in the processing list.<br>You can then add other processors to apply logic on the passed value for post-processing calculations in realtime.</p><p>Note: This virtual feed process list is executed on visualizations requests that use this virtual feed.</p>'),'short_name'=>'logxxx');
        //$list[54] = array(dgettext("process_messages","Source Daily (TBD)"),ProcessArg::FEEDID,"get_feed_data_day",1,DataType::DAILY,"Virtual", 'desc'=>"".dgettext("process_messages",'");

        $list[55] = array(dgettext("process_messages"," + source feed"),ProcessArg::FEEDID,"add_source_feed",0,DataType::UNDEFINED,dgettext("process_messages","Virtual"), 'desc'=>"".dgettext("process_messages",'<p>Add the specified feed.</p>'),'short_name'=>'+ sfeed');
        $list[56] = array(dgettext("process_messages"," - source feed"),ProcessArg::FEEDID,"sub_source_feed",0,DataType::UNDEFINED,dgettext("process_messages","Virtual"), 'desc'=>"".dgettext("process_messages",'<p>Subtract the specified feed.</p>'),'short_name'=>'- sfeed');
        $list[57] = array(dgettext("process_messages"," * source feed"),ProcessArg::FEEDID,"multiply_by_source_feed",0,DataType::UNDEFINED,dgettext("process_messages","Virtual"), 'desc'=>"".dgettext("process_messages",'<p>Multiply by specified feed.</p>'),'short_name'=>'x sfeed');
        $list[58] = array(dgettext("process_messages"," / source feed"),ProcessArg::FEEDID,"divide_by_source_feed",0,DataType::UNDEFINED,dgettext("process_messages","Virtual"), 'desc'=>"".dgettext("process_messages",'<p>Divide by specified feed. Returns NULL for zero values.</p>'),'short_name'=>'/ sfeed');
        $list[59] = array(dgettext("process_messages","1/ source feed"),ProcessArg::FEEDID,"reciprocal_by_source_feed",0,DataType::UNDEFINED,dgettext("process_messages","Virtual"), 'desc'=>"".dgettext("process_messages",'<p>Return the reciprical of the specified feed. Returns NULL for zero values.</p>'),'short_name'=>'?0 null');
        return $list;
    }
