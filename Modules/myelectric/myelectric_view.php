...
<div id="config" style="margin: 0px auto; max-width:320px; display:none">
    <div class="electric-title">My Electric config</div><br><br>
    <p><b style="color:#0699fa">Power feed:</b></p>
    <select id="powerfeed" style="width:290px"></select>
    <br><br>
    <p><b style="color:#0699fa">kWh feed:</b></p>
    <select id="kwhfeed" style="width:290px"></select>
    <br><br><button id="configsave" class="btn btn-primary">Save</button>
</div>
...
<script>
  ...
  powerfeed = config.powerfeed*1; 
  kwhfeed = config.kwhfeed*1; 
  ...
  function draw()
  {
    ...
    for (var x=0; x<daily.length; x++)
    {
      ...
      if (daily[x][1]>2000) ctx.fillText((daily[x][1]/1000).toFixed(0),10+x*(barwidth+10)+(barwidth/2),(bound.height-(daily[x][1]*0.024)*scale)+10);
    }
  }
