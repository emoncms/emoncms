...
function feed_controller()
{
    ...
    if ($route->format == 'html') {
        ...
        if (($route->action == "view" || $route->action == "list") && $session['write']) {
            ...
            return view("Modules/feed/Views/feedapi_view.php", array());
        } elseif (!$session['read']) {
            return ''; // empty strings force user back to login
        } else {
            return EMPTY_ROUTE; // this string displays error
        }
        ...
        if ($route->action == "list") {
            if ($session['read']) {
                ...
                } elseif (isset($_GET['userid']) && $_GET['userid'] != $session['userid']) {
        ...
        // http://emoncms.org/feed/fetch.json?ids=123,567,890
            ...
            return $result;
        } elseif ($route->action == "csvexport" && $session['write'] && isset($_GET['ids'])) {
        ...
        // Multi feed actions
        // ----------------------------------------------------------------------------
        } elseif (in_array($route->action, array("data","average"))) {
            ...
            if ($feed->exist($feedid)) { // if the feed exists
                ...
                if ($f['public'] || ($session['userid']>0 && $f['userid']==$session['userid'] && $session['read'])) {
                    if ($route->action == "timevalue") {
                        ...
                        return $feed->get_value($feedid); // null is a valid response
                    } elseif ($route->action == "get") {
                        return $feed->get_field($feedid, get('field')); // '/[^\w\s-]/'
                    } elseif ($route->action == "aget") {
                        return $feed->get($feedid);
                    } elseif ($route->action == "getmeta") {
                        return $feed->get_meta($feedid);
                    } elseif ($route->action == "setstartdate") {
                        return $feed->set_start_date($feedid, get('startdate'));
                    } elseif ($route->action == 'histogram') {
                        return $feed->histogram_get_power_vs_kwh($feedid, get('start'), get('end'));
                    } elseif ($route->action == 'kwhatpower') {
                        return $feed->histogram_get_kwhd_atpower($feedid, get('min'), get('max'));
                    } elseif ($route->action == 'kwhatpowers') {
                        return $feed->histogram_get_kwhd_atpowers($feedid, get('points'));
                    } elseif ($route->action == "csvexport") {
                        return $feed->csv_export($feedid, get('start'), get('end'), get('interval'), get('timeformat'));
                        ...
                        if ($f['engine']==Engine::MYSQL || $f['engine']==Engine::MYSQLMEMORY) {
                            ...
                            return $feed->phpfiwa_export($feedid, get('start'), get('layer'));
