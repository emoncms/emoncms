    ...
    else if ($route->format == 'json')
    {
        ...
        if ($route->action == "list")
        {
            if ($session['read']) {
                ...
                else if (isset($_GET['userid']) && $_GET['userid'] != $session['userid']) $result = $feed->get_user_public_feeds(prop('userid'));
            }
            else if (isset($_GET['userid'])) $result = $feed->get_user_public_feeds(prop('userid'));
            ...
            $route->format = "text";
            $result = $feed->get_id($session['userid'],prop("name"));
        } elseif ($route->action == "create" && $session['write']) {
            $result = $feed->create($session['userid'],prop('tag'),prop('name'),prop('datatype'),prop('engine'),json_decode(prop('options')));
        ...
        // http://emoncms.org/feed/fetch.json?ids=123,567,890
            ...
            $feedids = (array) (explode(",",(prop('ids'))));
            ...
            $result = $feed->csv_export_multi(prop('ids'),prop('start'),prop('end'),prop('interval'),prop('timeformat'),prop('name'));
        } else {
            $feedid = (int) prop('id');
            ...
            if ($feed->exist($feedid)) // if the feed exists
            {
                ...
                if ($f['public'] || ($session['userid']>0 && $f['userid']==$session['userid'] && $session['read']))
                {
                    if ($route->action == "timevalue") $result = $feed->get_timevalue($feedid);
                        ...
                        if (isset($_GET['interval'])) {
                            $result = $feed->get_data($feedid,prop('start'),prop('end'),prop('interval'),$skipmissing,$limitinterval);
                        } else if (isset($_GET['mode'])) {
                            if (isset($_GET['split'])) {
                                $result = $feed->get_data_DMY_time_of_day($feedid,prop('start'),prop('end'),prop('mode'),prop('split'));
                            } else {
                                $result = $feed->get_data_DMY($feedid,prop('start'),prop('end'),prop('mode'));
                            }
                        }
                    }
                    else if ($route->action == 'average') {
                        if (isset($_GET['interval'])) {
                            $result = $feed->get_average($feedid,prop('start'),prop('end'),prop('interval'));
                        } else if (isset($_GET['mode'])) {
                            $result = $feed->get_average_DMY($feedid,prop('start'),prop('end'),prop('mode'));
                        }
                    }
                    else if ($route->action == "value") $result = $feed->get_value($feedid); // null is a valid response
                    else if ($route->action == "get") $result = $feed->get_field($feedid,prop('field')); // '/[^\w\s-]/'
                    else if ($route->action == "aget") $result = $feed->get($feedid);
                    else if ($route->action == "getmeta") $result = $feed->get_meta($feedid);

                    else if ($route->action == 'histogram') $result = $feed->histogram_get_power_vs_kwh($feedid,prop('start'),prop('end'));
                    else if ($route->action == 'kwhatpower') $result = $feed->histogram_get_kwhd_atpower($feedid,prop('min'),prop('max'));
                    else if ($route->action == 'kwhatpowers') $result = $feed->histogram_get_kwhd_atpowers($feedid,prop('points'));
                    else if ($route->action == "csvexport") $result = $feed->csv_export($feedid,prop('start'),prop('end'),prop('interval'),prop('timeformat'),prop('name'));
                }
                ...
                if (isset($session['write']) && $session['write'] && $session['userid']>0 && $f['userid']==$session['userid'])
                {
                    ...
                    if ($route->action == 'set') {
                        $result = $feed->set_feed_fields($feedid,prop('fields'));

                    // Insert datapoint
                    } else if ($route->action == "insert") { 
                        $result = $feed->insert_data($feedid,time(),prop("time"),prop("value"));

                    // Update datapoint
                    } else if ($route->action == "update") {
                        if (isset($_GET['updatetime'])) $updatetime = prop("updatetime"); else $updatetime = time();
                        $result = $feed->update_data($feedid,$updatetime,prop("time"),prop('value'));
                    ...
                    // Process
                        ...
                        else if ($route->subaction == "set") $result = $feed->set_processlist($feedid, prop('processlist'));
                    ...
                    // Fast bulk uploader
                        ...
                        if (isset($_GET['start']) && isset($_GET['interval']) && isset($_GET['npoints'])) {
                            $result = $feed->upload_fixed_interval($feedid,prop("start"),prop("interval"),prop("npoints"));
                        } else if (isset($_GET['npoints'])) {
                            $result = $feed->upload_variable_interval($feedid,prop("npoints"));
                        }
                    }

                    if ($f['engine']==Engine::MYSQL || $f['engine']==Engine::MYSQLMEMORY) {
                        if ($route->action == "export") $result = $feed->mysqltimeseries_export($feedid,prop('start'));
                        else if ($route->action == "deletedatapoint") $result = $feed->mysqltimeseries_delete_data_point($feedid,prop('feedtime'));
                        else if ($route->action == "deletedatarange") $result = $feed->mysqltimeseries_delete_data_range($feedid,prop('start'),prop('end'));
                    } elseif ($f['engine']==Engine::PHPTIMESERIES) {
                        if ($route->action == "export") $result = $feed->phptimeseries_export($feedid,prop('start'));
                    } elseif ($f['engine']==Engine::PHPFIWA) {
                        if ($route->action == "export") $result = $feed->phpfiwa_export($feedid,prop('start'),prop('layer'));
                    } elseif ($f['engine']==Engine::PHPFINA) {
                        if ($route->action == "export") $result = $feed->phpfina_export($feedid,prop('start'));
                    }
                }
            }
            else
