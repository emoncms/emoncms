...
<div class="input-prepend input-append">
    <span class="add-on"><?php echo _("Show points"); ?></span>
    <span class="add-on"><input type="checkbox" id="show_points" ></span>
</div>
...
<div style="margin-top:10px">
    ...
    <!-- save csv -->
    <a id="save-csv" class="btn btn-warning" style="display:none"><?php echo _('Save'); ?></a>
</div>
...
<script id="source" language="javascript" type="text/javascript">
    ...
    if (feeds_by_id[feedid]!=undefined) {
    ...
    $('#okb').click(function() {
        ...
        $.ajax({
            ...
            data: "id=" + feedid + "&time=" + time + "&value=" + newvalue + "&skipbuffer=1",
            dataType: 'json',
            async: false,
            success: function() {}
        });
    ...
    $('#multiply-submit').click(function() {
        ...
        $.ajax({
            ...
            data: "id=" + feedid + "&start=" + parseInt(view.start * 0.001) + "&end=" + parseInt(view.end * 0.001) + "&value=" + multiplyvalue,
            dataType: 'json',
            async: false,
            success: function(result) {
                alert(result)
    ...
    $('#delete-button').click(function() {
        ...
        if (confirm("Are you sure you want to delete this data range?")) {
            $.ajax({
                ...
                data: "id=" + feedid + "&start=" + parseInt(view.start * 0.001) + "&end=" + parseInt(view.end * 0.001),
                dataType: 'json',
                async: true,
                success: function(result) {
                    vis_feed_data();
    ...
    $("#show_points").click(function() {
        options.series.points.show = $(this)[0].checked;
        redraw();
    });
    ...
    function resize() {
        ...
        var height = $(window).height() - $("#graph_bound").offset().top - 20 - 300;
        if (height<300) {
            height = 300;
        }
        $("#graph").height(height);
