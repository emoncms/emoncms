...
function formatColor(code) {
    return ['rgb(60,135,170)','rgb(50,200,50)','rgb(240,180,20)','rgb(255,125,20)','rgb(255,0,0)','rgb(150,150,150)'][code];
}

function formatTime(time, interval = 1) {
    const now   = Date.now();
    ...
    let text;
    if (!time || !isFinite(secs))          text = 'n/a';
    ...
    const ONE_MONTH = 3600 * 24 * 31;
    let code = 5;
    if (time && isFinite(secs)) {
        if (delta < 0)                          code = 0;
        else if (interval === 1) {
            if      (secs < 30)                 code = 1;
            else if (secs < 60)                 code = 2;
            else if (secs < 3600)               code = 3;
            else if (secs < ONE_MONTH)          code = 4;
        } else {
            if      (secs < interval *  3)      code = 1;
            else if (secs < interval *  6)      code = 2;
            else if (secs < interval * 12)      code = 3;
            else if (secs < ONE_MONTH)          code = 4;
        }
    }
    return { text, color: formatColor(code), code };
}
