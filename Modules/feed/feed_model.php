...
class Feed
{
    ...
    */
    public function create($userid, $tag, $name, $datatype, $engine, $options_in, $unit = '')
    {
        ...
        if (preg_replace('/[^\p{N}\p{L}_\s\-:]/u', '', $name)!=$name) {
            return array('success'=>false, 'message'=>'invalid characters in feed name');
        }
        if (preg_replace('/[^\p{N}\p{L}_\s\-:]/u', '', $tag)!=$tag) {
            return array('success'=>false, 'message'=>'invalid characters in feed tag');
        }
        ...
        if ($datatype==DataType::HISTOGRAM && $engine!=Engine::MYSQL) {
            $engine = Engine::MYSQL;
        }
    ...
    */
    ...
    public function get_data($feedid, $start, $end, $outinterval, $skipmissing, $limitinterval)
    {
        $feedid = (int) $feedid;
        if ($end<=$start) {
            return array('success'=>false, 'message'=>"Request end time before start time");
        }
        if (!$this->exist($feedid)) {
            return array('success'=>false, 'message'=>'Feed does not exist');
        }
    ...
    public function get_data_DMY($feedid, $start, $end, $mode)
    {
        ...
        if ($engine != Engine::PHPFINA && $engine != Engine::PHPTIMESERIES && $engine != Engine::MYSQL) {
            return array('success'=>false, 'message'=>"This request is only supported by PHPFina, PHPTimeseries AND MySQLTimeseries");
        }
    ...
    public function get_data_DMY_time_of_day($feedid, $start, $end, $mode, $split)
    {
        ...
        if ($engine != Engine::PHPFINA && $engine != Engine::MYSQL) {
            return array('success'=>false, 'message'=>"This request is only supported by PHPFina AND MySQLTimeseries");
        }
    ...
    public function csv_export_multi_prepare($feedids, $start, $end, $outinterval)
    {
        ...
        for ($i=0; $i<count($feedids); $i++) {
            ...
            if (isset($feedname['success']) && !$feedname['success']) {
                return $feedname;
            }
            $feeddata = $this->get_data($feedid, $start*1000, $end*1000, $outinterval, 0, 0);
            if (isset($feeddata['success']) && !$feeddata['success']) {
                return $feeddata;
            }
    ...
    */
    public function set_feed_fields($id, $fields)
    {
        ...
        if (isset($fields->unit)) {
            ...
            if (strlen($fields->unit) > 10) {
                return array('success'=>false, 'message'=>'feed unit too long');
            }
            if ($stmt = $this->mysqli->prepare("UPDATE feeds SET unit = ? WHERE id = ?")) {
                ...
                $stmt->close();
                if ($this->redis) {
                    $this->redis->hset("feed:$id", 'unit', $fields->unit);
                }
            }
        }
        ...
        if (isset($fields->public)) {
            ...
            if ($stmt = $this->mysqli->prepare("UPDATE feeds SET public = ? WHERE id = ?")) {
                ...
                $stmt->close();
                if ($this->redis) {
                    $this->redis->hset("feed:$id", 'public', $public);
                }
            }
        }
    ...
    */
    ...
    public function set_processlist($userid, $id, $processlist, $process_list)
    {
        ...
        foreach ($process_list as $key => $process) {
            if (isset($process['id_num'])) {
                $map[$process['id_num']] = $key;
            }
        }
        ...
        foreach ($pairs as $pair) {
            ...
            if (count($inputprocess)==2) {
                ...
                if (isset($map[$processkey])) {
                    $processkey = $map[$processkey];
                }
        ...
        if ($this->mysqli->affected_rows>0) {
            if ($this->redis) {
                $this->redis->hset("feed:$id", 'processList', $processlist_out);
            }
