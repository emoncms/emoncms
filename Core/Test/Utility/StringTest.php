 ...
 * @package       Cake.Test.Case.Utility
 */
class StringTest extends EmonTestCase {
 ...
 */
	public function testWordWrapUnicodeAware() {
		...
		$expected = <<<TEXT
ÐÐ¾ Ð²Ð¸Ð¼ Ð¾Ð¼Ð½Ð¸ÑŽÐ¼ Ñ„Ð°ÐºÑ‘Ð»ÑŒÐ¸ÑÐ¸ ÑÐ»Ñ‹ÐºÑ‚Ñ€Ð°Ð¼,
Ð¼ÑŽÐ½Ñ‹Ñ€Ñ Ð»ÑÐ³Ñ‹Ñ€Ñ‹ Ð²ÐµÐºÐ¶ Ñ‹Ñ‚. Ð’Ñ‹Ð»ÑŒÑ‘Ñ‚ ÐºÐ²ÑŽ
Ð°Ð½Ð´Ð¾ Ð½ÑŽÐ¼ÐºÐ²ÑƒÐ°Ð¼ Ñ‚Ñ‹ ÐºÑŽÐ¼. Ð—Ñ‹Ð´ ÑÑŽ Ñ€Ñ‹Ð±ÑŽ
Ð¼.
TEXT;
		...
		$expected = <<<TEXT
ÐÐ¾ Ð²Ð¸Ð¼ Ð¾Ð¼Ð½Ð¸ÑŽÐ¼ Ñ„Ð°ÐºÑ‘Ð»ÑŒÐ¸ÑÐ¸ ÑÐ»Ñ‹ÐºÑ‚Ñ€Ð°Ð¼,
Ð¼ÑŽÐ½Ñ‹Ñ€Ñ Ð»ÑÐ³Ñ‹Ñ€Ñ‹ Ð²ÐµÐºÐ¶ Ñ‹Ñ‚. Ð’Ñ‹Ð»ÑŒÑ‘Ñ‚
ÐºÐ²ÑŽÐ°Ð½Ð´Ð¾ Ð½ÑŽÐ¼ÐºÐ²ÑƒÐ°Ð¼ Ñ‚Ñ‹ ÐºÑŽÐ¼. Ð—Ñ‹Ð´ ÑÑŽ
Ñ€Ñ‹Ð±ÑŽÐ¼.
TEXT;
		$this->assertTextEquals($expected, $result, 'Text not wrapped.');
	}
 ...
 */
	public function testTruncate() {
		...
		$text7 = 'El moÃ±o estÃ¡ en el lugar correcto. Eso fue lo que dijo la niÃ±a, Â¿habrÃ¡ dicho la verdad?';
		$text8 = 'Vive la R' . chr(195) . chr(169) . 'publique de France';
		$text9 = 'ÐÐžÐŸÐ Ð¡Ð¢Ð£Ð¤Ð¥Ð¦Ð§Ð¨Ð©ÐªÐ«Ð¬Ð­Ð®Ð¯Ð°Ð±Ð²Ð³Ð´ÐµÐ¶Ð·Ð¸Ð¹ÐºÐ»Ð¼Ð½Ð¾Ð¿Ñ€ÑÑ‚ÑƒÑ„Ñ…Ñ†Ñ‡ÑˆÑ‰ÑŠÑ‹ÑŒ';
		...
		$this->assertSame($this->Text->truncate($text7, 15), 'El moÃ±o estÃ¡...');
		$this->assertSame($this->Text->truncate($text8, 15), 'Vive la R' . chr(195) . chr(169) . 'pu...');
		$this->assertSame($this->Text->truncate($text9, 10), 'ÐÐžÐŸÐ Ð¡Ð¢Ð£...');
		...
		$text = '<p><span style="font-size: medium;">El biÃ³grafo de Steve Jobs, Walter
Isaacson, explica porquÃ© Jobs le pidiÃ³ que le hiciera su biografÃ­a en
este artÃ­culo de El PaÃ­s.</span></p>
<p><span style="font-size: medium;"><span style="font-size:
large;">Por quÃ© Steve era distinto.</span></span></p>
...
<p><span style="font-size: medium;">Ya se ha publicado la biografÃ­a de
Steve Jobs escrita por Walter Isaacson  "<strong>Steve Jobs by Walter
Isaacson</strong>", aquÃ­ os dejamos la direcciÃ³n de amazon donde
podeÃ­s adquirirla.</span></p>
...
Jobs-Walter-Isaacson/dp/1451648537</a></span></p>';
		...
		$expected = '<p><span style="font-size: medium;">El biÃ³grafo de Steve Jobs, Walter
Isaacson, explica porquÃ© Jobs le pidiÃ³ que le hiciera su biografÃ­a en
este artÃ­culo de El PaÃ­s.</span></p>
<p><span style="font-size: medium;"><span style="font-size:
large;">Por quÃ© Steve era distinto.</span></span></p>
 ...
 */
	public function testTail() {
		$text1 = 'The quick brown fox jumps over the lazy dog';
		$text2 = 'Heiz&ouml;lr&uuml;cksto&szlig;abd&auml;mpfung';
		$text3 = 'El moÃ±o estÃ¡ en el lugar correcto. Eso fue lo que dijo la niÃ±a, Â¿habrÃ¡ dicho la verdad?';
		$text4 = 'Vive la R' . chr(195) . chr(169) . 'publique de France';
		$text5 = 'ÐÐžÐŸÐ Ð¡Ð¢Ð£Ð¤Ð¥Ð¦Ð§Ð¨Ð©ÐªÐ«Ð¬Ð­Ð®Ð¯Ð°Ð±Ð²Ð³Ð´ÐµÐ¶Ð·Ð¸Ð¹ÐºÐ»Ð¼Ð½Ð¾Ð¿Ñ€ÑÑ‚ÑƒÑ„Ñ…Ñ†Ñ‡ÑˆÑ‰ÑŠÑ‹ÑŒ';
		...
		$this->assertEquals('...Ñ†Ñ‡ÑˆÑ‰ÑŠÑ‹ÑŒ', $result);

		$result = $this->Text->tail($text5, 6, array('ellipsis' => ''));
		$this->assertEquals('Ñ‡ÑˆÑ‰ÑŠÑ‹ÑŒ', $result);
	}
 ...
 */
	public function testHighlight() {
		...
		$phrases = array('saÃŸ', 'cafÃ©', 'Ã¼bergang');
		$result = $this->Text->highlight($text, $phrases, array('format' => '<b>\1</b>'));
		$this->assertEquals($expected, $result);
	}
