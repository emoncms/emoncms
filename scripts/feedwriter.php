...
if (!$settings['redis']['enabled']) {
    $log->error("Error: setting must be true: redis_enabled");
    die;
}
if (!$settings['feed']['redisbuffer']['enabled']) {
    $log->error("Error: setting must be true: settings['feed']['redisbuffer']['enabled']");
    die;
}
if (!$settings['feed']['redisbuffer']['sleep'] || (int)$settings['feed']['redisbuffer']['sleep'] < 1) {
    $log->error("Error: setting must be > 0 : settings['feed']['redisbuffer']['sleep']");
    die;
}
    ...
    if ($settings['redis']['enabled']) {
        ...
        if (!$redis->connect($settings['redis']['host'], $settings['redis']['port'])) {
            $log->error("Cannot connect to redis at ".$settings['redis']['host'].":".$settings['redis']['port']);
            die('Check log\n');
        }
        ...
        if (!empty($settings['redis']['auth'])) {
            if (!$redis->auth($settings['redis']['auth'])) {
                $log->error("Cannot connect to redis at ".$settings['redis']['host'].", autentication failed");
                die('Check log\n');
            }
        }
    } else {
